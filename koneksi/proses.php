...
if($_GET['id'] == 'booking')
{
    ...
    $cek_plat = "SELECT id_mobil FROM mobil WHERE merk = '$nama_mobil' AND no_plat='$no_plat' limit 1";
    $hasils = $koneksi->query($cek_plat)->fetch();
    $data[] = time();
    $data[] = $hasils['id_mobil'];
    $data[] = $_POST['ktp'];
    $data[] = $_POST['nama'];
    $data[] = $_POST['alamat'];
    $data[] = $_POST['telepon'];
    $data[] = $_POST['tanggal'];
    $data[] = $_POST['tanggalPengembalian'];
    $data[] = $_POST['totalHari'];
    $data[] = $total_harga;
    $data[] = "Belum Bayar";
    $data[] = date('Y-m-d');
    ...
    window.location="../admin/booking/booking.php?id='.time().'";</script>'; 
}
